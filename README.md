...
- JSP 기본요소
  ...
  - **스크립트 요소(Scripting Elements)**
    ...
    - 스크립틀릿(Scriptlet)
      ...
      서블릿의 단점 : HTML 응답을 만들어 내기 위해서는 각각의 HTML 태그를 PrintWriter 등의 클래스를 사용해 출력 객체를 생성한 후 일일이 출력해주어야 했음 --> 코드가 길어질 수록 유지 보수가 어려움

      HTML로 된 일반 코드 부분은 HTML 파일처럼 그대로 사용하고 자바 코드로 이루어진 비즈니스 로직 부분은 <% .. %>로 표현되는 스크립틀릿 태그를 사용하여 구분하여 쉽게 HTML 응답을 만들어 냄
    ...
    - 표현식(Expression)
      ...
      선언문 또는 스크립틀릿 태그에서 선언된 변수 값이나 메소드의 리턴값을 스크립틀릿 태그 외부에서 출력하기 위해 사용되는 방법
